...
class RadiantUltraScanner(ctk.CTk):
    ...
    def setup_sidebar(self):
        ...
        # Action Buttons
        self.scan_btn = ctk.CTkButton(self.sidebar, text="START SCAN", command=self.run_process, height=55, corner_radius=12, fg_color=COLORS["accent"], hover_color=COLORS["accent_hover"], font=("Inter", 14, "bold"))
    ...
    def setup_main_view(self):
        ...
        self.proceed_btn = ctk.CTkButton(self.main_view, text="PROCEED TO OCR ENGINE", command=self.proceed, width=350, height=60, corner_radius=30, fg_color=COLORS["success"], font=("Inter", 16, "bold"))
    ...
    def rotate_image(self):
        if self.selected_file:
            ...
            self.display_image()
            self.log_message(f"Calibrated image axis: {self.rotation}°")
    ...
    def run_process(self):
        ...
        self.load_info.pack(expand=True, pady=(0, 10)); self.prog.pack(pady=10); self.prog.start()
        threading.Thread(target=lambda: [time.sleep(2), self.after(0, self.finish_scan)], daemon=True).start()

    def finish_scan(self):
        self.overlay.place_forget(); self.prog.stop()
        self.log_message("Buffer digitized successfully.")

    def proceed(self):
        if self.selected_file: self.log_message("Handing over to OCR pipeline...")
